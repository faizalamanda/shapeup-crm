...
export default function EditExpensePage() {
  ...
  // Load Active Business Profile, Accounts, Suppliers, and the Expense itself
  useEffect(() => {
    async function loadData() {
      try {
        ...
        if (businessId) {
          ...
          // Set form fields
          setFormCategoryAccountId(expData.category_account_id || '')
          setFormPaymentAccountId(expData.payment_account_id || '')
          setFormAmount(expData.amount?.toString() || '')
          setFormDate(expData.date || '')
          setFormDescription(expData.description || '')
          setFormVendorName(expData.vendor_name || '')
          setVendorSearch(expData.vendor_name || '')
          setFormAttachmentUrl(expData.attachment_url || '')
          setPaymentStatus(expData.payment_status || 'paid')
          setDueDate(expData.due_date || '')
          setAmountPaid(expData.amount_paid?.toString() || '')
          setHasPayments(expData.payment_status === 'paid' || (expData.expense_payments && expData.expense_payments.length > 0))
        }
      } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const errData = await res.json()
        throw new Error(errData.error || 'Gagal memperbarui pengeluaran')
      }
      ...
      <div className="flex items-center justify-between border-b border-gray-200 pb-4">
        <div>
          ...
          <p className="text-xs text-gray-500 font-medium">
            Sesuaikan detail pencatatan pengeluaran usaha untuk {activeBizName}
          </p>
        </div>
      </div>

      {hasPayments && (
        <div className="p-4 text-xs font-bold text-rose-800 bg-rose-50 border border-rose-200 rounded-xl">
          ⚠️ Pengeluaran ini tidak dapat diubah karena sudah memiliki riwayat pembayaran cicilan. Silakan hapus pembayaran cicilan terlebih dahulu untuk mengedit atau menyesuaikan nominal transaksi ini.
        </div>
      )}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full p-2.5 border border-gray-300 rounded-lg text-xs font-semibold text-gray-800 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 bg-white disabled:bg-gray-50 disabled:text-gray-400"
                value={formDate}
            ...
            <div className="relative" ref={vendorDropdownRef}>
              ...
              <input
                ...
                className="w-full p-2.5 border border-gray-300 rounded-lg text-xs font-semibold text-gray-800 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 bg-white disabled:bg-gray-50 disabled:text-gray-400"
                value={vendorSearch}
                onFocus={() => !hasPayments && setIsVendorDropdownOpen(true)}
                onChange={e => {
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            <button
              ...
              className="w-full p-2.5 border border-gray-300 rounded-lg text-xs font-semibold text-gray-800 bg-white flex justify-between items-center outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-left cursor-pointer disabled:bg-gray-50 disabled:text-gray-400"
            >
            ...
            <div>
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  disabled={hasPayments}
                  onClick={() => !hasPayments && setPaymentStatus('paid')}
                  className={`flex-1 py-2 px-3 text-xs font-bold rounded-lg border text-center transition-all cursor-pointer disabled:opacity-50 ${
                    paymentStatus === 'paid'
                      ? 'bg-emerald-600 border-emerald-600 text-white shadow-xs'
                      : 'bg-white border-gray-205 text-gray-600 hover:bg-gray-100'
                  }`}
                >
                  🟢 Langsung Lunas
                </button>
                <button
                  type="button"
                  disabled={hasPayments}
                  onClick={() => !hasPayments && setPaymentStatus('unpaid')}
                  className={`flex-1 py-2 px-3 text-xs font-bold rounded-lg border text-center transition-all cursor-pointer disabled:opacity-50 ${
                ...
                <button
                  type="button"
                  disabled={hasPayments}
                  onClick={() => !hasPayments && setPaymentStatus('partial')}
                  className={`flex-1 py-2 px-3 text-xs font-bold rounded-lg border text-center transition-all cursor-pointer disabled:opacity-50 ${
            ...
            {paymentStatus === 'paid' && (
              <div className="animate-in slide-in-from-top-2 duration-200">
                ...
                <select
                  required
                  disabled={hasPayments}
                  className="w-full p-2.5 border border-gray-300 rounded-lg text-xs font-semibold text-gray-800 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 bg-white disabled:bg-gray-50"
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              disabled={hasPayments}
              className="w-full p-2.5 border border-gray-300 rounded-lg text-xs font-semibold text-gray-800 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 bg-white resize-none disabled:bg-gray-50 disabled:text-gray-400"
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                type="button"
                disabled={compressing || hasPayments}
          ...
          <div className="flex gap-3 justify-end border-t border-gray-150 pt-5">
            ...
            <button
              type="submit"
              disabled={submitLoading || compressing || hasPayments}
