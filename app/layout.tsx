...
export default function RootLayout({
  ...
  // Fungsi untuk mengecek menu aktif agar ada highlight biru
    ...
    <html lang="en">
      <body className={inter.className}>
        <div className="flex min-h-screen bg-slate-50">
          ...
          {/* SIDEBAR FIXED */}
          <aside className="w-72 bg-[#0F172A] text-slate-400 flex flex-col fixed h-full shadow-2xl">
            {/* LOGO SECTION */}
            <div className="p-8">
            ...
            <nav className="flex-1 px-4 space-y-1.5 mt-4">
              <p className="px-4 text-[10px] font-black uppercase tracking-[0.2em] text-slate-600 mb-4">Main Menu</p>
              ...
              <Link href="/orders" className={`flex items-center gap-3 px-4 py-3.5 rounded-2xl transition-all group ${isActive('/orders') ? 'bg-blue-600 text-white shadow-xl shadow-blue-500/20' : 'hover:bg-slate-800/50 hover:text-slate-200'}`}>
                <span className={`text-xl ${isActive('/orders') ? 'opacity-100' : 'opacity-50 group-hover:opacity-100'}`}>📦</span>
                <span className="text-sm font-bold tracking-tight">Daftar Pesanan</span>
              </Link>
              ...
              <Link href="/orders/new" className={`flex items-center gap-3 px-4 py-3.5 rounded-2xl transition-all group ${isActive('/orders/new') ? 'bg-blue-600 text-white shadow-xl shadow-blue-500/20' : 'hover:bg-slate-800/50 hover:text-slate-200'}`}>
                <span className={`text-xl ${isActive('/orders/new') ? 'opacity-100' : 'opacity-50 group-hover:opacity-100'}`}>➕</span>
                <span className="text-sm font-bold tracking-tight">Input Manual</span>
              </Link>
            </nav>

            {/* BOTTOM SECTION: SETTINGS */}
            <div className="p-4 mt-auto border-t border-slate-800/50 mb-4">
