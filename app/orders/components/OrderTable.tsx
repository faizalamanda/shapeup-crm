          ...
          <tbody className="divide-y divide-slate-100 text-[11px] text-center">
            {orders.map((o) => (
              <tr key={o.id} className="hover:bg-[#fffdf0] transition-colors group">
                ...
                <td className="px-4 py-2 text-slate-500 text-left truncate max-w-[200px]" title={o.items_json}>
                  {o.items_json ? (typeof o.items_json === 'string' ? o.items_json : JSON.stringify(o.items_json)) : '-'}
                </td>
                ...
                <td className="px-4 py-2 text-right">{formatIDR(o.cart_tax || 0)}</td>
                ...
                <td className="px-4 py-2">
                  <a href={`https://wa.me/${o.phone || o.wa_number}`} target="_blank" className="inline-block bg-[#25D366] text-white p-1 rounded-sm hover:scale-110 transition-transform">💬</a>
                </td>
              </tr>
            ))}
          </tbody>
