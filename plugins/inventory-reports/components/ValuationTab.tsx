...
export default function ValuationTab({ stockItems, moves, loading }: ValuationTabProps) {
  ...
  const sortedBreakdown = useMemo(() => {
    return [...valuationResult.itemBreakdown].sort((a, b) => {
              ...
              <tr>
                ...
                >
                  <div className="flex items-center">
                    Nama Produk & SKU {renderSortArrow('productName')}
                  </div>
                </th>
                <th
                ...
                >
                  <div className="flex items-center justify-end">
                    Stok Fisik (Qty) {renderSortArrow('qtyOnHand')}
                  </div>
                </th>
                <th
                ...
                >
                  <div className="flex items-center justify-end">
                    Harga Modal Standar {renderSortArrow('standardCost')}
                  </div>
                </th>
                <th
                  onClick={() => handleSort('unitCostCalculated')}
                  className="py-3 px-4 text-right cursor-pointer hover:bg-[#ECECE8] transition-colors select-none"
                >
                  <div className="flex items-center justify-end">
                    Unit Cost {valuationResult.method} {renderSortArrow('unitCostCalculated')}
                  </div>
                </th>
                <th
                  onClick={() => handleSort('totalValueCalculated')}
                  className="py-3 px-4 text-right cursor-pointer hover:bg-[#ECECE8] transition-colors select-none"
                >
                  <div className="flex items-center justify-end">
                    Total Nilai {valuationResult.method} {renderSortArrow('totalValueCalculated')}
                  </div>
                </th>
                <th
                  onClick={() => handleSort('varianceVsStandard')}
                  className="py-3 px-4 text-right cursor-pointer hover:bg-[#ECECE8] transition-colors select-none"
                >
                  <div className="flex items-center justify-end">
                    Selisih / Varians {renderSortArrow('varianceVsStandard')}
                  </div>
                </th>
              </tr>
            </thead>
                ...
                <tr>
                  <td colSpan={6} className="py-8 text-center text-[#82827A]">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <span>Menghitung penilaian persediaan...</span>
                    </div>
                  </td>
                </tr>
